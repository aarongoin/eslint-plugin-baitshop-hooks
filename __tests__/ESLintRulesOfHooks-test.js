...
const ReactHooksESLintRule = require('../lib/RulesOfHooks');
...
const baitshopTests = {
  valid: [
    `
      // Valid because Hook classes can use hooks inside their onRender method
      class MyCustomHook extends Hook {
        onRender() {
          useHook();
        }
      }
    ...
    `
      // Valid because Hook classes can use hooks inside their onRender method
      class MyCustomHook extends Baitshop.Hook {
        onRender() {
          useHook();
        }
      }
  ...
  invalid: [
    {
      code: `
        // Invalid because hook used inside static method
        class MyCustomHook extends Hook {
          static onRender() {
            useHook();
          }
    ...
    {
      code: `
        // Invalid because hook used inside a conditional Hook class method
        class MyCustomHook extends Hook {
          onChange() {
            useHook();
          }
...
eslintTester.run('baitshop-hooks', ReactHooksESLintRule, tests);
// custom baitshop tests
eslintTester.run('baitshop-hooks', ReactHooksESLintRule, baitshopTests);
