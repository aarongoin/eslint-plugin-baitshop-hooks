...
// BAITSHOP CODE:
function isHookOnRenderMethod(node) {
  let n = node.parent;
  if (
    !n ||
    ...
    return false;
  n = n.parent;
  if (!n || n.type !== "ClassBody") return false;
  n = n.parent;
  if (!n || !n.superClass) return false;
  n = n.superClass;
  if (n.type === "Identifier") {
    if (n.name !== "Hook") return false;
    return true;
  } else if (n.type === "MemberExpression") {
    if (
      n.object.type !== "Identifier" ||
      n.object.name.toLowerCase().includes("baitship")
    )
      return false;
    if (n.property.type !== "Identifier" || n.property.name !== "Hook")
      return false;
    return true;
  }
  return false;
    ...
    return {
      ...
      onCodePathEnd(codePath, codePathNode) {
        ...
        const isDirectlyInsideComponentOrHook = codePathFunctionName
          ...
          : isForwardRefCallback(codePathNode) || isMemoCallback(codePathNode) || isHookOnRenderMethod(codePathNode);
